...
class BluetoothConnection {
  ...
  ///
  /// You should use some encoding to receive string in your `.listen` callback, for servo_app `ascii.decode(data)` or `utf8.encode(data)`.
  Stream<Uint8List>? input;

  /// Stream sink used to write to the remote Bluetooth device
  ///
  /// You should use some encoding to send string, for servo_app `.add(ascii.encode('Hello!'))` or `.add(utf8.encode('Cześć!))`.
...
class _BluetoothStreamSink<Uint8List> extends StreamSink<Uint8List> {
  ...
  ///
  /// You should use some encoding to send string, for servo_app `ascii.encode('Hello!')` or `utf8.encode('Cześć!)`.
  ///
